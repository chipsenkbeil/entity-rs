use crate::utils;
use entity_macros_data::{StructEnt, StructEntEdge, StructEntEdgeKind};
use proc_macro2::TokenStream;
use quote::{format_ident, quote};
use syn::{Ident, Path, Type};

pub fn do_derive_ent_typed_edges(root: Path, ent: StructEnt) -> TokenStream {
    let name = &ent.ident;
...
fn fn_typed_id_getter(edge: &StructEntEdge) -> TokenStream {
    let name = &edge.name;
    let ty = &edge.ty;

    let method_name = match edge.kind {
        StructEntEdgeKind::Maybe | StructEntEdgeKind::One => format_ident!("{}_id", name),
        StructEntEdgeKind::Many => format_ident!("{}_ids", name),
    };
    let return_type = match edge.kind {
        StructEntEdgeKind::Maybe | StructEntEdgeKind::One => quote! { #ty },
        StructEntEdgeKind::Many if edge.use_id_slice => {
            let inner_t = utils::get_innermost_type(ty);
            quote! { &[#inner_t] }
        }
        StructEntEdgeKind::Many => quote! { &#ty },
    };
    let inner_return = match edge.kind {
        StructEntEdgeKind::Maybe | StructEntEdgeKind::One => quote! { self.#name },
        StructEntEdgeKind::Many => quote! { &self.#name },
    };
...
fn fn_typed_id_setter(edge: &StructEntEdge) -> TokenStream {
    let name = &edge.name;
    let ty = &edge.ty;

    let method_name = match edge.kind {
        StructEntEdgeKind::Maybe | StructEntEdgeKind::One => format_ident!("set_{}_id", name),
        StructEntEdgeKind::Many => format_ident!("set_{}_ids", name),
    };
...
fn fn_typed_load_edge(root: &Path, edge: &StructEntEdge) -> TokenStream {
    match edge.kind {
        StructEntEdgeKind::Maybe => fn_typed_load_edge_of_maybe(
            root,
        ...
        StructEntEdgeKind::One => fn_typed_load_edge_of_one(
            root,
            &format_ident!("load_{}", edge.name),
            &edge.name,
            &edge.ent_ty,
        ...
        StructEntEdgeKind::Many => fn_typed_load_edge_of_many(
            root,
            &format_ident!("load_{}", edge.name),
            &edge.name,
            &edge.ent_ty,
